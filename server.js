...
app.post('/adminlogin', (req, res) => {
  ...
  if (username === 'Admin' && password === 'AdminPassword123') {
    ...
    res.redirect('/admin'); // Redirect to the admin dashboard or desired page
  } else {
    // Authentication failed
    res.redirect('/adminlogin'); // Redirect back to the login form
  }
})
...
const authenticateAdmin = (req, res, next) => {
  if (req.session.isAuthenticated) {
    ...
    res.redirect('/adminlogin'); // User is not authenticated, redirect to the login form
  }
};
